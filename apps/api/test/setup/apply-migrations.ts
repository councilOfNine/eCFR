/**
 * Give every Worker-side test in this package a real, fully migrated, empty D1.
 *
 ...
 *
 * NOTE: the repo-root config has a twin of this file at test/setup/apply-migrations.ts, which
 * serves the packages/db suite as well as this one. This copy exists because vitest resolves
 * `setupFiles` inside the project root, and `pnpm --filter @ecfr-atlas/api test` roots at
 * apps/api. Reaching two directories up would work only by widening vite's filesystem
 * allowlist, which is a worse trade than ninety lines that are covered by the same tests.
 */
